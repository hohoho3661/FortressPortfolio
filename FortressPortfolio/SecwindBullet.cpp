...
bool SecwindBullet::Init(float _x, float _y)
{
	bulletImg = IMAGEMANAGER->findImage(TEXT("SecwindBullet"));
	bulletAni = ANIMANAGER->FindAni(TEXT("S_Bullet"));
	bulletAni->start();
...
void SecwindBullet::Render(HDC hdc)
{
	// 총알 그리기
...
void SecwindBullet::Release()
{
}

void SecwindBullet::BulletFire(float tankCenterX, float tankCenterY, float tankAngle, float _cannonPower)
{
	// 힘 전달
	speedX += _cannonPower;
	speedY += _cannonPower;
...
void SecwindBullet::BulletMove(int _wind, int _dir)
{
	gravity = 0.5f;
